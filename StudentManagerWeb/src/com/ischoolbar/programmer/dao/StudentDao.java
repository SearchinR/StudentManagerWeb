...
public class StudentDao extends BaseDao {
	public boolean addStudent(Student student){
		...
		sql += ",'" + student.getQq() + "',null"+",'"+student.getAddress()+"')";
		return update(sql);
	}
	...
	public Student getStudent(int id){
		...
		try {
			if(resultSet.next()){
				...
				student.setSn(resultSet.getString("address"));
				return student;
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public int getMNum(){
		int total = 0;
		String sql = "select count(*)as total from s_student where sex='ÄÐ'";
		ResultSet resultSet = query(sql.replaceFirst("and", "where"));
		try {
			while(resultSet.next()){
				total = resultSet.getInt("total");
	...
	public int getWNum(){
		int total = 0;
		String sql = "select count(*)as total from s_student where sex='Å®'";
		ResultSet resultSet = query(sql.replaceFirst("and", "where"));
		try {
			while(resultSet.next()){
				total = resultSet.getInt("total");
