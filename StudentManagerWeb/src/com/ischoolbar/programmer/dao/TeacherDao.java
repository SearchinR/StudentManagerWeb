...
public class TeacherDao extends BaseDao {
	public boolean addTeacher(Teacher teacher){
		...
		sql += ",'" + teacher.getQq() + "',null,'"+teacher.getEducation()+"')";
	...
	public List<Teacher> getTeacherList(Teacher teacher,Page page){
		...
		try {
			while(resultSet.next()){
				...
				t.setEducation(resultSet.getString("education"));
				ret.add(t);
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public int getBenNum() {
		int total = 0;
		String sql = "select count(*)as total from s_teacher where education='本科生' ";
		ResultSet resultSet = query(sql.replaceFirst("and", "where"));
		try {
			while(resultSet.next()){
				total = resultSet.getInt("total");
	...
	public int getYanNum() {
		int total = 0;
		String sql = "select count(*)as total from s_teacher where education='研究生'";
		ResultSet resultSet = query(sql.replaceFirst("and", "where"));
		try {
			while(resultSet.next()){
				total = resultSet.getInt("total");
	...
	public int getBoNum() {
		int total = 0;
		String sql = "select count(*)as total from s_teacher  where education='博士'";
		ResultSet resultSet = query(sql.replaceFirst("and", "where"));
		try {
			while(resultSet.next()){
				total = resultSet.getInt("total");
	...
	public Teacher login(String name ,String password){
		...
		try {
			if(resultSet.next()){
				...
				teacher.setSex(resultSet.getString("sex"));
				teacher.setSn(resultSet.getString("sn"));
				teacher.setEducation(resultSet.getString("eduction"));
				return teacher;
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
